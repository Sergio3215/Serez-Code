...
impl MirLowerer {
    ...
    fn lower_function(&mut self, func: &HirFunction) -> MirFunction {
        let params: Vec<(String, SzType)> = func
            .params
            .iter()
...
impl<'a> FnLowerer<'a> {
    ...
    fn fresh_label(&mut self, p: &str) -> String {
        self.mir.fresh_label(p)
    }
            ...
            HirStmt::Assign(lval, value) => {
                ...
                match lval {
                    ...
                    HirLValue::Field { object, field } => {
                        ...
                        self.cur_instrs
                            .push(MirInstr::FieldStore(obj, field.clone(), val));
                    }
                }
            }
    ...
    fn lower_expr(&mut self, expr: &HirExpr) -> MirVal {
        match expr {
            ...
            HirExpr::Call { name, args, .. } => {
                ...
                let t = self.fresh_temp();
                self.cur_instrs
                    .push(MirInstr::Call(Some(t), name.clone(), arg_vals));
                MirVal::Temp(t)
            }
            ...
            HirExpr::MethodCall {
                ...
                let arg_vals: Vec<MirVal> = args.iter().map(|a| self.lower_expr(a)).collect();
                let t = self.fresh_temp();
                self.cur_instrs.push(MirInstr::MethodCall(
                    Some(t),
                    obj_val,
                    method.clone(),
                    arg_vals,
            ...
            HirExpr::Field { object, name, .. } => {
                ...
                let t = self.fresh_temp();
                self.cur_instrs
                    .push(MirInstr::FieldLoad(t, obj, name.clone()));
                MirVal::Temp(t)
            }
            ...
            HirExpr::New { class, args } => {
                ...
                let t = self.fresh_temp();
                self.cur_instrs
                    .push(MirInstr::New(t, class.clone(), arg_vals));
                MirVal::Temp(t)
            }
            ...
            HirExpr::Array { elements, .. } => {
                ...
                let t = self.fresh_temp();
                self.cur_instrs
                    .push(MirInstr::Call(Some(t), "__sz_array_new".to_string(), vals));
                MirVal::Temp(t)
            }
            ...
            HirExpr::If {
                ...
                self.cur_instrs
                    .push(MirInstr::Store(result_var.clone(), then_val));
                self.seal(Terminator::Jump(merge_lbl.clone()));
                ...
                self.cur_instrs
                    .push(MirInstr::Store(result_var.clone(), else_val));
                self.seal(Terminator::Jump(merge_lbl.clone()));
...
mod tests {
    ...
    fn int_var(name: &str) -> HirExpr {
        HirExpr::Var(name.to_string(), SzType::Int)
    }
    ...
    fn let_const_emits_store_instruction() {
        let f = lower(vec![HirStmt::Let {
            name: "x".into(),
            ty: SzType::Int,
            value: HirExpr::LitInt(42),
            is_const: false,
        }]);
        ...
        assert!(
            entry
                .instrs
                .iter()
                .any(|i| { matches!(i, MirInstr::Store(n, MirVal::ConstInt(42)) if n == "x") })
        );
    }
    ...
    fn let_bool_emits_store_const_bool() {
        let f = lower(vec![HirStmt::Let {
            name: "flag".into(),
            ty: SzType::Bool,
            value: HirExpr::LitBool(true),
            is_const: false,
        }]);
        assert!(
            f.blocks[0].instrs.iter().any(|i| {
    ...
    fn arithmetic_binop_emits_binop_and_store() {
        ...
        assert!(instrs.iter().any(|i| matches!(
            i,
            MirInstr::BinOp(_, HirBinOp::Add, MirVal::ConstInt(3), MirVal::ConstInt(4))
        )));
        // Store("r", Temp(t))
        assert!(
            instrs
                .iter()
                .any(|i| matches!(i, MirInstr::Store(n, MirVal::Temp(_)) if n == "r"))
        );
    }
    ...
    fn load_var_emits_load_instruction() {
        ...
        assert!(
            instrs
                .iter()
                .any(|i| matches!(i, MirInstr::Load(_, n) if n == "x"))
        );
    }
    ...
    fn return_value_emits_return_terminator() {
        ...
        assert!(
            f.blocks
                .iter()
                .any(|b| { matches!(&b.term, Terminator::Return(Some(MirVal::ConstInt(7)))) })
        );
    }
    ...
    fn for_update_block_jumps_back_to_cond() {
        ...
        let update = f
            .blocks
            .iter()
            .find(|b| b.label.starts_with("for_update"))
            .unwrap();
    ...
    fn out_statement_emits_out_instruction() {
        ...
        assert!(
            f.blocks[0]
                .instrs
                .iter()
                .any(|i| { matches!(i, MirInstr::Out(MirVal::ConstStr(s)) if s == "hello") })
        );
    }
    ...
    fn conditional_expr_creates_if_then_else_merge_blocks() {
        ...
        assert!(
            merge
                .instrs
                .iter()
                .any(|i| matches!(i, MirInstr::Store(n, _) if n == "v"))
        );
    }
    ...
    fn fibonacci_program_lowers_to_valid_mir() {
        ...
        assert!(f.blocks.iter().any(|b| {
            b.instrs
                .iter()
                .any(|i| matches!(i, MirInstr::Call(_, nm, _) if nm == "fib"))
        }));
    }
    ...
    fn sum_n_for_loop_program() {
                ...
                body: vec![
                    ...
                    HirStmt::For {
                        ...
                        body: vec![HirStmt::Assign(
                            HirLValue::Var("sum".into()),
                            add(int_var("sum"), int_var("i")),
                        )],
                    },
        ...
        let body = f
            .blocks
            .iter()
            .find(|b| b.label.starts_with("for_body"))
            .unwrap();
        assert!(
            body.instrs
                .iter()
                .any(|i| matches!(i, MirInstr::Store(n, _) if n == "sum"))
        );
    }
    ...
    fn countdown_while_with_break() {
                ...
                body: vec![
                    ...
                    HirStmt::While {
                        cond: HirExpr::BinOp {
                            op: HirBinOp::Gt,
                            left: Box::new(int_var("n")),
                            right: Box::new(HirExpr::LitInt(0)),
                            ty: SzType::Bool,
                        },
                        body: vec![
                            HirStmt::Assign(
        ...
        assert!(
            f.blocks
                .iter()
                .any(|b| matches!(&b.term, Terminator::Jump(lbl) if *lbl == exit_lbl))
        );
    }
}
