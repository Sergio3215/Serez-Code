...
impl super::Evaluator {
    pub(super) fn eval_media_namespace(&mut self, dot_call: &ast::DotCallExpression) -> EvalResult {
        if let Some(error) = self.require_permission("Media", "Media") {
            return error;
        }
        ...
        match dot_call.method.as_str() {
            ...
            "playSound" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Media.playSound(path) requires 1 argument");
                }
                let path = match self.gui_str_arg(&dot_call.arguments[0]) {
                    Some(p) => p,
                    None => {
                        return self
                            .rt_err_kind("TypeError", "Media.playSound path must be a string");
                    }
                };
                ...
                if self.media.is_none() {
                    ...
                    if self.media.is_none() {
                        return self.rt_err_kind(
                            "MediaError",
                            "Media.playSound: no audio output device available",
                        );
                    }
                }
            ...
            "isPlaying" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Media.isPlaying(id) requires 1 argument");
                }
                let id = match self.gui_int_arg(&dot_call.arguments[0]) {
                    Some(v) => v,
                    None => {
                        return self
                            .rt_err_kind("TypeError", "Media.isPlaying id must be an integer");
                    }
                };
                let playing = self
                    .media
            ...
            "stop" => {
                ...
                let id = match self.gui_int_arg(&dot_call.arguments[0]) {
                    Some(v) => v,
                    None => {
                        return self.rt_err_kind("TypeError", "Media.stop id must be an integer");
                    }
                };
                let existed = self
                    .media
            ...
            "setVolume" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Media.setVolume(id, volume) requires 2 arguments",
                    );
                }
                ...
                let (id, vol) = match (id, vol) {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Media.setVolume requires (int, int)");
                    }
                };
                let ok = self
                    .media
