        ...
        match dot_call.method.as_str() {
            ...
            "clamp" => {
                ...
                match (
                    ...
                    self.resolve(mn).cloned(),
                    self.resolve(mx).cloned(),
                ) {
                    (
            ...
            "abs" | "sqrt" | "floor" | "ceil" | "round" | "log" | "log2" | "log10" | "sin"
            | "cos" | "tan" | "asin" | "acos" | "atan" | "trunc" | "exp" => {
        ...
        match dot_call.method.as_str() {
            "exists" => {
                ...
                let path = match self.resolve(pr).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "File.exists requires a string path");
                    }
                };
                ...
                match std::fs::metadata(&path) {
                    ...
                    Ok(meta) if meta.len() > MAX_FILE_READ => {
                        return self.fatal_err_kind(
                            "ResourceError",
                            format!("File '{}' exceeds maximum read size (256 MiB)", path),
                        );
                    }
                    Err(e) => {
            ...
            "write" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "File.write(path, content) requires 2 arguments",
                    );
                }
                let pr = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
            ...
            "create" => {
                ...
                let path = match self.resolve(pr).cloned() {
                    Some(ObjectData::Str(s)) => s,
                    _ => {
                        return self.rt_err_kind("TypeError", "File.create requires a string path");
                    }
                };
            ...
            "read_asBinary" => {
                ...
                let path = match self.resolve(pr).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "File.read_asBinary requires a string path");
                    }
                };
            ...
            "write_asBinary" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "File.write_asBinary(path, bytes) requires 2 arguments",
                    );
                }
                let pr = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let path = match self.resolve(pr).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "File.write_asBinary path must be a string");
                    }
                };
                let bytes_data = match self.resolve(br).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "File.write_asBinary bytes must be an array",
                        );
                    }
                };
                ...
                for owned in bytes_data {
                    match owned {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "File.write_asBinary: each byte must be int 0-255",
                            );
                        }
                    }
                }
            ...
            "listDir" => {
                ...
                let path = match self.resolve(pr).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "File.listDir requires a string path");
                    }
                };
            ...
            "delete" => {
                if let Some(error) =
                    self.require_unsafe("File.delete", "it permanently removes files")
                {
                    return error;
                }
                if dot_call.arguments.len() != 1 {
                ...
                let path = match self.resolve(pr).cloned() {
                    Some(ObjectData::Str(s)) => s,
                    _ => {
                        return self.rt_err_kind("TypeError", "File.delete requires a string path");
                    }
                };
            ...
            "rename" => {
                if let Some(error) =
                    self.require_unsafe("File.rename", "it modifies the filesystem")
                {
                    return error;
                }
                if dot_call.arguments.len() != 2 {
                ...
                let from = match self.resolve(fr).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "File.rename: 'from' must be a string");
                    }
                };
                let to = match self.resolve(tr).cloned() {
                    Some(ObjectData::Str(s)) => s,
                    _ => {
                        return self.rt_err_kind("TypeError", "File.rename: 'to' must be a string");
                    }
                };
        ...
        match dot_call.method.as_str() {
            ...
            "pretty" => {
                if dot_call.arguments.is_empty() || dot_call.arguments.len() > 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "JSON.pretty(value, [indent]) requires 1 or 2 arguments",
                    );
                }
                let vr = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let indent = if dot_call.arguments.len() == 2 {
                    ...
                    match self.resolve(ir) {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "JSON.pretty indent must be a non-negative integer",
                            );
                        }
                    }
                } else {
