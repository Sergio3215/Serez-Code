...
impl Ctx {
    ...
    fn eff_fill(&self) -> Option<[u8; 4]> {
        self.fill
            .map(|mut c| {
                c[3] = (c[3] as f32 * self.fill_opacity * self.opacity)
                    .round()
    ...
    fn eff_stroke(&self) -> Option<[u8; 4]> {
        self.stroke
            .map(|mut c| {
                c[3] = (c[3] as f32 * self.stroke_opacity * self.opacity)
                    .round()
                    .clamp(0.0, 255.0) as u8;
                c
            })
...
fn apply_paint(c: &mut Ctx, k: &str, v: &str) {
    match k {
        ...
        "opacity" => {
            if let Some(n) = parse_num(v) {
                c.opacity = n.clamp(0.0, 1.0);
            }
        }
        "fill-opacity" => {
            if let Some(n) = parse_num(v) {
                c.fill_opacity = n.clamp(0.0, 1.0);
            }
        }
        "stroke-opacity" => {
            if let Some(n) = parse_num(v) {
                c.stroke_opacity = n.clamp(0.0, 1.0);
            }
        }
        "fill-rule" => {
            c.fill_rule = if v == "evenodd" {
                FillRule::EvenOdd
            } else {
                FillRule::Winding
            };
        ...
        match e.name.as_str() {
            ...
            "path" | "rect" | "circle" | "ellipse" | "line" | "polyline" | "polygon" => {
                ...
                if let Some(path) = build_path(&e.name, &e.attrs) {
                    ...
                    let fill = if e.name == "line" || e.name == "polyline" {
                        None
                    } else {
                        ctx.eff_fill()
                    };
...
fn arc_to(
    ...
    let segs = (dtheta.abs() / (std::f32::consts::PI / 2.0))
        .ceil()
        .max(1.0) as usize;
