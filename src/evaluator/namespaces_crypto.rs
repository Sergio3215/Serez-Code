...
impl super::Evaluator {
    pub(super) fn eval_crypto_namespace(
        &mut self,
        ...
        match dot_call.method.as_str() {
            ...
            "hmacSha256" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Crypto.hmacSha256(key, data) requires 2 arguments",
                    );
                }
                ...
                let data =
                    match self.eval_to_string(&dot_call.arguments[1], "Crypto.hmacSha256 data") {
                        Ok(v) => v,
                        Err(e) => return e,
                    };
            ...
            "ed25519Keypair" => {
                if !dot_call.arguments.is_empty() {
                    return self
                        .rt_err_kind("TypeError", "Crypto.ed25519Keypair() takes no arguments");
                }
                let mut seed = [0u8; 32];
                ...
                let entries = vec![
                    (
                        OwnedValue::Str("private".to_string()),
                        OwnedValue::Str(to_hex(&seed)),
                    ),
                    (
                        OwnedValue::Str("public".to_string()),
            ...
            "ed25519Sign" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Crypto.ed25519Sign(privateHex, message) requires 2 arguments",
                    );
                }
                let priv_hex = match self
                    .eval_to_string(&dot_call.arguments[0], "Crypto.ed25519Sign privateHex")
                {
                    Ok(v) => v,
                    Err(e) => return e,
                };
                let message = match self
                    .eval_to_string(&dot_call.arguments[1], "Crypto.ed25519Sign message")
                {
                    Ok(v) => v,
                    Err(e) => return e,
                };
            ...
            "ed25519Verify" => {
                ...
                let pub_hex = match self
                    .eval_to_string(&dot_call.arguments[0], "Crypto.ed25519Verify publicHex")
                {
                    Ok(v) => v,
                    Err(e) => return e,
                };
                let message = match self
                    .eval_to_string(&dot_call.arguments[1], "Crypto.ed25519Verify message")
                {
                    Ok(v) => v,
                    Err(e) => return e,
                };
                let sig_hex = match self
                    .eval_to_string(&dot_call.arguments[2], "Crypto.ed25519Verify signatureHex")
                {
                    Ok(v) => v,
                    Err(e) => return e,
                };
            ...
            _ => self.rt_err_kind(
                "TypeError",
                format!("Unknown Crypto method '{}'", dot_call.method),
            ),
        }
    }
    ...
    pub(super) fn require_one_string(
        &mut self,
        args: &[ast::Expression],
        ctx: &str,
...
fn base64_encode(data: &[u8]) -> String {
    ...
    for chunk in data.chunks(3) {
        ...
        out.push(if n > 1 {
            B64_TABLE[((combined >> 6) & 0x3f) as usize] as char
        } else {
            '='
        });
        out.push(if n > 2 {
            B64_TABLE[(combined & 0x3f) as usize] as char
        } else {
            '='
        });
    }
