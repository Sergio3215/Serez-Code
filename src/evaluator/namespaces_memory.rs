...
impl super::Evaluator {
    pub(super) fn eval_memory_namespace(
        &mut self,
        ...
        match dot_call.method.as_str() {
            ...
            "sizeof" => {
                    ...
                    EvalResult::Value(r) => match self.resolve(r).cloned() {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Memory.sizeof() argument must be a string type name",
                            );
                        }
                    },
                ...
                let size: i64 = match type_name.as_str() {
                    ...
                    other => {
                        return self.rt_err_kind(
                            "MemoryError",
                            format!("Memory.sizeof() — unknown type '{}'", other),
                        );
                    }
                };
            ...
            "alloc" => {
                if let Some(error) = self.require_unsafe("Memory.alloc", "it allocates raw memory")
                {
                    return error;
                }
                if dot_call.arguments.len() != 1 {
                ...
                if n == 0 {
                    return self
                        .rt_err_kind("TypeError", "Memory.alloc() size must be at least 1 byte");
                }
                if n > 256 * 1024 * 1024 {
            ...
            "free" => {
                if let Some(error) = self.require_unsafe("Memory.free", "it releases raw memory") {
                    return error;
                }
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Memory.free(handle) requires 1 argument");
                }
            ...
            "size" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Memory.size(handle) requires 1 argument");
                }
                let id = match self.eval_memory_id(&dot_call.arguments[0]) {
                    Ok(v) => v,
                    Err(e) => return e,
            ...
            "read" => {
                if let Some(error) = self.require_unsafe("Memory.read", "it reads raw memory") {
                    return error;
                }
                if dot_call.arguments.len() != 3 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Memory.read(handle, offset, type) requires 3 arguments",
                    );
                }
                let id = match self.eval_memory_id(&dot_call.arguments[0]) {
                    Ok(v) => v,
                ...
                let buf = match self.memory_heap.get(&id) {
                    ...
                    None => {
                        return self.rt_err_kind(
                            "MemoryError",
                            format!("Memory.read() — no allocation with handle {}", id),
                        );
                    }
                };
                match type_name.as_str() {
                    "bool" | "int8" | "byte" | "uint8" => {
                        if offset >= buf.len() {
                            return self.rt_err_kind(
                                "MemoryError",
                                format!(
                                    "Memory.read() offset {} out of bounds (size {})",
                                    offset,
                                    buf.len()
                                ),
                            );
                    ...
                    "int16" | "uint16" => {
                        if offset + 2 > buf.len() {
                            return self.rt_err_kind(
                                "MemoryError",
                                format!(
                                    "Memory.read() offset {} out of bounds for int16 (size {})",
                                    offset,
                                    buf.len()
                                ),
                            );
                    ...
                    "int32" | "uint32" => {
                        if offset + 4 > buf.len() {
                            return self.rt_err_kind(
                                "MemoryError",
                                format!(
                                    "Memory.read() offset {} out of bounds for int32 (size {})",
                                    offset,
                                    buf.len()
                                ),
                            );
                    ...
                    "int64" | "int" | "uint64" => {
                        if offset + 8 > buf.len() {
                            return self.rt_err_kind(
                                "MemoryError",
                                format!(
                                    "Memory.read() offset {} out of bounds for int64 (size {})",
                                    offset,
                                    buf.len()
                                ),
                            );
                    ...
                    "float32" => {
                        if offset + 4 > buf.len() {
                            return self.rt_err_kind(
                                "MemoryError",
                                format!(
                                    "Memory.read() offset {} out of bounds for float32 (size {})",
                                    offset,
                                    buf.len()
                                ),
                            );
                    ...
                    "float64" | "decimal" => {
                        if offset + 8 > buf.len() {
                            return self.rt_err_kind(
                                "MemoryError",
                                format!(
                                    "Memory.read() offset {} out of bounds for float64 (size {})",
                                    offset,
                                    buf.len()
                                ),
                            );
                    ...
                    other => self.rt_err_kind(
                        "MemoryError",
                        format!("Memory.read() — unknown type '{}'", other),
                    ),
                }
            }
            ...
            "write" => {
                if let Some(error) = self.require_unsafe("Memory.write", "it writes raw memory") {
                    return error;
                }
                if dot_call.arguments.len() != 4 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Memory.write(handle, offset, type, value) requires 4 arguments",
                    );
                }
                let id = match self.eval_memory_id(&dot_call.arguments[0]) {
                    Ok(v) => v,
                ...
                let buf = match self.memory_heap.get_mut(&id) {
                    ...
                    None => {
                        return self.rt_err_kind(
                            "MemoryError",
                            format!("Memory.write() — no allocation with handle {}", id),
                        );
                    }
                };
            ...
            "copy" => {
                if let Some(error) = self.require_unsafe("Memory.copy", "it writes raw memory") {
                    return error;
                }
                if dot_call.arguments.len() != 3 {
                    return self
                        .rt_err_kind("TypeError", "Memory.copy(src, dst, n) requires 3 arguments");
                }
                ...
                let src_bytes: Vec<u8> = match self.memory_heap.get(&src_id) {
                    Some(b) => {
                        if n > b.len() {
                            return self.rt_err_kind(
                                "MemoryError",
                                format!("Memory.copy() — src size {} < n {}", b.len(), n),
                            );
                        }
                        b[..n].to_vec()
                    }
                    None => {
                        return self.rt_err_kind(
                            "MemoryError",
                            format!("Memory.copy() — no src allocation {}", src_id),
                        );
                    }
                };
                ...
                let dst_len = match self.memory_heap.get(&dst_id) {
                    ...
                    None => {
                        return self.rt_err_kind(
                            "MemoryError",
                            format!("Memory.copy() — no dst allocation {}", dst_id),
                        );
                    }
                };
                if n > dst_len {
            ...
            "fill" => {
                if let Some(error) = self.require_unsafe("Memory.fill", "it writes raw memory") {
                    return error;
                }
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Memory.fill(handle, byte_value) requires 2 arguments",
                    );
                }
                let id = match self.eval_memory_id(&dot_call.arguments[0]) {
                    Ok(v) => v,
                    ...
                    EvalResult::Value(r) => match self.resolve(r).cloned() {
                        Some(ObjectData::Integer(n)) => {
                            if n < 0 || n > 255 {
                                return self.rt_err_kind(
                                    "TypeError",
                                    "Memory.fill() byte value must be 0–255",
                                );
                            }
                            n as u8
                        }
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Memory.fill() byte value must be an integer 0–255",
                            );
                        }
                    },
                    EvalResult::Throw(v) => return EvalResult::Throw(v),
                ...
                match self.memory_heap.get_mut(&id) {
                    ...
                    None => {
                        return self.rt_err_kind(
                            "MemoryError",
                            format!("Memory.fill() — no allocation with handle {}", id),
                        );
                    }
                }
                EvalResult::Value(self.null_ref)
            ...
            "offsetOf" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Memory.offsetOf(class, field) requires 2 arguments",
                    );
                }
                let class_name = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => match self.resolve(r).cloned() {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Memory.offsetOf() class must be a string",
                            );
                        }
                    },
                    EvalResult::Throw(v) => return EvalResult::Throw(v),
                ...
                let field_name = match self.eval_expression(&dot_call.arguments[1]) {
                    EvalResult::Value(r) => match self.resolve(r).cloned() {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Memory.offsetOf() field must be a string",
                            );
                        }
                    },
                    EvalResult::Throw(v) => return EvalResult::Throw(v),
                ...
                let class = match self.class_registry.get(&class_name).cloned() {
                    ...
                    None => {
                        return self.rt_err_kind(
                            "MemoryError",
                            format!("Memory.offsetOf() — unknown class '{}'", class_name),
                        );
                    }
                };
                ...
                match field_idx {
                    ...
                    None => self.rt_err_kind(
                        "MemoryError",
                        format!(
                            "Memory.offsetOf() — class '{}' has no field '{}'",
    ...
    fn memory_write_typed(
        ...
        match type_name {
            ...
            "int16" | "uint16" => {
                if offset + 2 > buf.len() {
                    return Err(format!(
                        "offset {} out of bounds for int16 (size {})",
                        offset,
                        buf.len()
                    ));
                }
            ...
            "int32" | "uint32" => {
                if offset + 4 > buf.len() {
                    return Err(format!(
                        "offset {} out of bounds for int32 (size {})",
                        offset,
                        buf.len()
                    ));
                }
            ...
            "int64" | "int" | "uint64" => {
                if offset + 8 > buf.len() {
                    return Err(format!(
                        "offset {} out of bounds for int64 (size {})",
                        offset,
                        buf.len()
                    ));
                }
            ...
            "float32" => {
                if offset + 4 > buf.len() {
                    return Err(format!(
                        "offset {} out of bounds for float32 (size {})",
                        offset,
                        buf.len()
                    ));
                }
            ...
            "float64" | "decimal" => {
                if offset + 8 > buf.len() {
                    return Err(format!(
                        "offset {} out of bounds for float64 (size {})",
                        offset,
                        buf.len()
                    ));
                }
