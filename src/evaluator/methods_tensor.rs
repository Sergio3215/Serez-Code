...
impl super::Evaluator {
    ...
    pub(super) fn eval_new_tensor(&mut self, new_expr: &ast::NewExpression) -> EvalResult {
        let args = match &new_expr.args {
            ...
            ast::NewArgs::Fields(_) => {
                return self.rt_err_kind(
                    "TypeError",
                    "Tensor constructor requires positional arguments: new Tensor([shape], fill)",
                );
            }
        };
        if args.is_empty() {
            return self.rt_err_kind(
                "TypeError",
                "Tensor() requires at least a shape argument like [2, 3]",
            );
        }
        ...
        let total = match self.checked_tensor_element_count(&shape) {
            Ok(total) => total,
        ...
        match dot_call.method.as_str() {
            "zeros" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Tensor.zeros([shape]) requires 1 argument");
                }
            ...
            "ones" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Tensor.ones([shape]) requires 1 argument");
                }
                let shape = match self.eval_shape_expr(&dot_call.arguments[0]) {
                    Ok(s) => s,
                    Err(e) => return e,
                ...
                let n = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => match self.resolve(r) {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Tensor.eye(n) requires a positive integer",
                            );
                        }
                    },
                    EvalResult::Throw(v) => return EvalResult::Throw(v),
            ...
            _ => self.rt_err_kind(
                "TypeError",
                format!("Unknown Tensor static method '{}'", dot_call.method),
            ),
        }
    }
    ...
    pub(super) fn eval_tensor_method(
        ...
        match dot_call.method.as_str() {
            "shape" => {
                let owned: Vec<OwnedValue> = shape
                    .iter()
            ...
            "set" => {
                if dot_call.arguments.len() < 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.set() requires index arg(s) + value, e.g. set(0, 1, val)",
                    );
                }
            ...
            "reshape" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Tensor.reshape([shape]) requires 1 argument");
                }
                ...
                if new_total != data.len() {
                    return self.rt_err_kind(
                        "TensorError",
                        format!(
                            "Tensor.reshape() — shape has {} elements but tensor has {}",
                            new_total,
                            data.len()
                        ),
                    );
            ...
            "transpose" => {
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TensorError",
                        "Tensor.transpose() only supported for 2D tensors",
                    );
                }
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Transpose {
                            in_id,
                            rows,
                            cols,
                        },
                ...
                match self.resolve(arg_ref).cloned() {
                    Some(ObjectData::Tensor {
                        ...
                        if shape.len() != 1 || s2.len() != 1 {
                            return self.rt_err_kind(
                                "TensorError",
                                "Tensor.dot() only works on 1D tensors (use matmul for 2D)",
                            );
                        }
                        if shape[0] != s2[0] {
                            return self.rt_err_kind(
                                "TensorError",
                                format!("Tensor.dot() length mismatch: {} vs {}", shape[0], s2[0]),
                            );
                        }
                ...
                match self.resolve(arg_ref).cloned() {
                    Some(ObjectData::Tensor {
                        ...
                        if shape.len() != 2 || s2.len() != 2 {
                            return self
                                .rt_err_kind("TypeError", "Tensor.matmul() requires 2D tensors");
                        }
                        ...
                        if self.ad_recording {
                            ...
                            self.ad_push(
                                out_ref,
                                crate::evaluator::namespaces_autodiff::TapeOp::MatMul {
                                    a_id,
                                    b_id,
                                    a_rows: m,
                                    a_cols: k,
            ...
            "relu" => {
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Relu {
                            in_id,
                            cached_input: cached,
                        },
                    );
            ...
            "sigmoid" => {
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Sigmoid {
                            in_id,
                            cached_output: new_data,
                        },
                    );
                }
                EvalResult::Value(out_ref)
            ...
            "tanh" => {
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Tanh {
                            in_id,
                            cached_output: new_data,
                        },
                    );
            ...
            "softmax" => {
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Softmax {
                            in_id,
                            rows,
                            cols,
                            cached_out: new_data,
            ...
            "sqrt" => {
                for &x in &data {
                    if x < 0.0 {
                        return self.rt_err_kind(
                            "TensorError",
                            format!("Tensor.sqrt() — negative value {}", x),
                        );
                    }
                }
            ...
            "log" => {
                for &x in &data {
                    if x <= 0.0 {
                        return self.rt_err_kind(
                            "TensorError",
                            format!("Tensor.log() — non-positive value {}", x),
                        );
                    }
                }
                ...
                let exp = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => match self.resolve(r) {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Tensor.pow() exponent must be a number",
                            );
                        }
                    },
                    EvalResult::Throw(v) => return EvalResult::Throw(v),
            ...
            "norm" => {
                    ...
                    match self.eval_expression(&dot_call.arguments[0]) {
                        EvalResult::Value(r) => match self.resolve(r) {
                            ...
                            _ => {
                                return self.rt_err_kind(
                                    "TypeError",
                                    "Tensor.norm() order must be an integer",
                                );
                            }
                        },
                        EvalResult::Throw(v) => return EvalResult::Throw(v),
                ...
                let result = match order {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TensorError",
                            format!("Tensor.norm() supports order 1 or 2, got {}", order),
                        );
                    }
                };
                EvalResult::Value(self.alloc(ObjectData::Decimal(result)))
            ...
            "broadcastAdd" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Tensor.broadcastAdd(bias) requires 1 argument");
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TensorError",
                        "Tensor.broadcastAdd() only supported for 2D tensors",
                    );
                }
                let (rows, cols) = (shape[0], shape[1]);
                ...
                let bias_data = match self.resolve(bias_ref).cloned() {
                    Some(ObjectData::Tensor {
                        ...
                        if bs != vec![cols] {
                            return self.rt_err_kind(
                                "TensorError",
                                format!(
                                    "Tensor.broadcastAdd() bias shape {:?} must match last dim {}",
                                    bs, cols
                                ),
                            );
                        }
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.broadcastAdd() argument must be a 1D Tensor",
                        );
                    }
                };
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::BroadcastAdd {
                            mat_id,
                            bias_id,
                            rows,
                            cols,
            ...
            "broadcastMul" | "broadcastSub" | "broadcastDiv" => {
                if dot_call.arguments.len() != 1 {
                    return self.rt_err_kind(
                        "TypeError",
                        format!("Tensor.{}() requires 1 argument", dot_call.method),
                    );
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TensorError",
                        format!("Tensor.{}() only supported for 2D tensors", dot_call.method),
                    );
                }
                let (rows, cols) = (shape[0], shape[1]);
                ...
                let rhs_data = match self.resolve(rhs_ref).cloned() {
                    Some(ObjectData::Tensor {
                        ...
                        if rs != vec![cols] {
                            return self.rt_err_kind(
                                "TensorError",
                                format!(
                                    "Tensor.{}() rhs shape {:?} must match last dim {}",
                                    dot_call.method, rs, cols
                                ),
                            );
                        }
                        rd
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            format!("Tensor.{}() argument must be a 1D Tensor", dot_call.method),
                        );
                    }
                };
                if dot_call.method == "broadcastDiv" {
                    for v in &rhs_data {
                        if *v == 0.0 {
                            return self.rt_err_kind(
                                "TensorError",
                                "Tensor.broadcastDiv() division by zero in rhs",
                            );
                        }
                    }
                }
                ...
                if self.ad_recording && dot_call.method == "broadcastMul" {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::BroadcastMul {
                            mat_id,
                            rhs_id,
                            rows,
                            cols,
            ...
            "sumAxis" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Tensor.sumAxis(axis) requires 1 argument");
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TensorError",
                        "Tensor.sumAxis() only supported for 2D tensors",
                    );
                }
                let (rows, cols) = (shape[0], shape[1]);
                ...
                let axis = match self.resolve(ax_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.sumAxis() axis must be 0 or 1");
                    }
                };
                if axis == 0 {
            ...
            "meanAxis" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Tensor.meanAxis(axis) requires 1 argument");
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TensorError",
                        "Tensor.meanAxis() only supported for 2D tensors",
                    );
                }
                let (rows, cols) = (shape[0], shape[1]);
                let ax_ref = match self.eval_expression(&dot_call.arguments[0]) {
                ...
                let axis = match self.resolve(ax_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.meanAxis() axis must be 0 or 1");
                    }
                };
                if axis == 0 {
                    let mut out = vec![0.0f64; cols];
            ...
            "slice" => {
                ...
                let (start, end) = match (self.resolve(r0).cloned(), self.resolve(r1).cloned()) {
                    (Some(ObjectData::Integer(a)), Some(ObjectData::Integer(b))) => {
                        (a as usize, b as usize)
                    }
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.slice() arguments must be integers");
                    }
                };
                if start > end || end > data.len() {
                    return self.rt_err_kind(
                        "TensorError",
                        format!(
                            "Tensor.slice() invalid range {}..{} for tensor of length {}",
                            start,
                            end,
                            data.len()
                        ),
            ...
            "concat" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.concat(other, axis) requires 2 arguments",
                    );
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TensorError",
                        "Tensor.concat() only supported for 2D tensors",
                    );
                }
                ...
                let axis = match self.resolve(ax_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.concat() axis must be 0 or 1");
                    }
                };
                let (rows, cols) = (shape[0], shape[1]);
                        ...
                        if axis == 0 {
                            if os.len() != 2 || os[1] != cols {
                                return self.rt_err_kind(
                                    "TensorError",
                                    format!(
                                        "Tensor.concat(axis=0) column mismatch: {} vs {}",
                                        os.get(1).copied().unwrap_or(0),
                                        cols
                                    ),
                                );
                            }
                            ...
                            if os.len() != 2 || os[0] != rows {
                                return self.rt_err_kind(
                                    "TensorError",
                                    format!(
                                        "Tensor.concat(axis=1) row mismatch: {} vs {}",
                                        os.get(0).copied().unwrap_or(0),
                                        rows
                                    ),
                                );
                            }
            ...
            "scale" => {
                ...
                let s = match self.resolve(s_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.scale() argument must be a number");
                    }
                };
            ...
            "leaky_relu" => {
                ...
                let alpha = match self.resolve(a_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.leaky_relu() alpha must be a number",
                        );
                    }
                };
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::LeakyRelu {
                            in_id,
                            cached_input,
                            alpha,
                        },
            ...
            "gelu" => {
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Gelu {
                            in_id,
                            cached_input,
                        },
                    );
            ...
            "elu" => {
                let alpha = if dot_call.arguments.is_empty() {
                    1.0
                } else {
                    match self.eval_expression(&dot_call.arguments[0]) {
                        EvalResult::Value(r) => match self.resolve(r) {
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Elu {
                            in_id,
                            cached_input,
                            alpha,
                        },
            ...
            "swish" | "silu" => {
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Swish {
                            in_id,
                            cached_input,
                            cached_sigmoid,
                        },
            ...
            "mish" => {
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Mish {
                            in_id,
                            cached_input,
                        },
                    );
            ...
            "leaky_relu_tracked" => {
                if dot_call.arguments.len() != 1 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.leaky_relu_tracked(alpha) requires 1 argument",
                    );
                }
                let a_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let alpha = match self.resolve(a_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.leaky_relu_tracked() alpha must be a number",
                        );
                    }
                };
                let cached_input = data.clone();
            ...
            "avg_pool2d" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.avg_pool2d(kernel, stride) requires 2 arguments",
                    );
                }
                if shape.len() != 4 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.avg_pool2d() input must be 4D [N, H, W, C]",
                    );
                }
                            ...
                            for c in 0..ch {
                                ...
                                for kh in 0..kernel {
                                    for kw in 0..kernel {
                                        ...
                                        let iw = ow * stride + kw;
                                        s += data
                                            [nb * in_h * in_w * ch + ih * in_w * ch + iw * ch + c];
                                    }
                                }
                                out_data[nb * out_h * out_w * ch + oh * out_w * ch + ow * ch + c] =
                                    s / pool_area;
                            }
                        }
                    }
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::AvgPool2d {
                            in_id,
                            kernel,
                            stride,
            ...
            "unsqueeze" => {
                ...
                let dim = match self.resolve(d_ref) {
                    Some(ObjectData::Integer(n)) => {
                        ...
                        if d < 0 || d > ndim {
                            return self.rt_err_kind(
                                "TensorError",
                                format!(
                                    "unsqueeze dim {} out of range for {}D tensor",
                                    n,
                                    shape.len()
                                ),
                            );
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "unsqueeze dim must be an integer");
                    }
                };
            ...
            "squeeze" => {
                ...
                let new_shape: Vec<usize> = if dot_call.arguments.is_empty() {
                    ...
                    let dim = match self.resolve(d_ref) {
                        ...
                        _ => {
                            return self.rt_err_kind("TypeError", "squeeze dim must be an integer");
                        }
                    };
            ...
            "permute" => {
                    ...
                    Err(_) => match self.resolve(axes_ref).cloned() {
                        Some(ObjectData::Array { elements, .. }) => elements
                            ...
                            .filter_map(|e| match e {
                                OwnedValue::Integer(n) => Some(*n as usize),
                                _ => None,
                            })
                            .collect(),
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Tensor.permute() axes must be an array",
                            );
                        }
                    },
                };
                if axes.len() != shape.len() {
                    return self.rt_err_kind(
                        "TensorError",
                        format!(
                            "Tensor.permute() axes length {} must match tensor ndim {}",
                            axes.len(),
                            shape.len()
                        ),
                    );
            ...
            "broadcastTo" => {
                ...
                if dot_call.arguments.len() != 1 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.broadcastTo([shape]) requires 1 argument",
                    );
                }
                ...
                match Self::broadcast_data(&data, &shape, &target_shape) {
                    ...
                    None => self.rt_err_kind(
                        "TensorError",
                        format!(
                            "Tensor.broadcastTo() incompatible shapes {:?} → {:?}",
                            shape, target_shape
                        ),
                    ),
                }
            ...
            "broadcastAddNd" => {
                ...
                if dot_call.arguments.len() != 1 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.broadcastAddNd(other) requires 1 argument",
                    );
                }
                let other_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let (other_data, other_shape) = match self.resolve(other_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.broadcastAddNd() argument must be a Tensor",
                        );
                    }
                };
                let out_shape = match Self::broadcast_shape(&shape, &other_shape) {
                    Some(s) => s,
                    None => {
                        return self.rt_err_kind("TensorError", format!("Tensor.broadcastAddNd() shapes {:?} and {:?} are not broadcastable", shape, other_shape));
                    }
                };
            ...
            "broadcastMulNd" => {
                if dot_call.arguments.len() != 1 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.broadcastMulNd(other) requires 1 argument",
                    );
                }
                let other_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let (other_data, other_shape) = match self.resolve(other_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.broadcastMulNd() argument must be a Tensor",
                        );
                    }
                };
                let out_shape = match Self::broadcast_shape(&shape, &other_shape) {
                    ...
                    None => {
                        return self.rt_err_kind(
                            "TensorError",
                            "Tensor.broadcastMulNd() shapes not broadcastable",
                        );
                    }
                };
                let a_data = Self::broadcast_data(&data, &shape, &out_shape).unwrap();
            ...
            "bmm" => {
                ...
                if shape.len() != 3 {
                    return self.rt_err_kind(
                        "TypeError",
                        format!(
                            "Tensor.bmm() requires 3D tensors [B,N,M], got {}D",
                            shape.len()
                        ),
                    );
                }
                ...
                let (other_data, other_shape) = match self.resolve(other_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.bmm() argument must be a Tensor");
                    }
                };
                if other_shape.len() != 3
                    || other_shape[0] != shape[0]
                    || other_shape[1] != shape[2]
                {
                    return self.rt_err_kind(
                        "TensorError",
                        format!(
                            "Tensor.bmm() shape mismatch: {:?} @ {:?}",
                            shape, other_shape
                        ),
                    );
                }
                ...
                for bi in 0..b {
                    for i in 0..n {
                        for l in 0..m {
                            for j in 0..k {
                                out_data[bi * n * k + i * k + j] += data[bi * n * m + i * m + l]
                                    * other_data[bi * m * k + l * k + j];
                            }
                        }
                    }
                }
            ...
            "reduceSum" => {
                ...
                if dot_call.arguments.is_empty() {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.reduceSum(axis) requires at least 1 argument",
                    );
                }
                let ax_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let axis = match self.resolve(ax_ref) {
                    Some(ObjectData::Integer(n)) => {
                        ...
                        if a < 0 || a >= nd {
                            return self.rt_err_kind(
                                "TensorError",
                                format!("reduceSum axis {} out of range", n),
                            );
                        }
                        a as usize
                    }
                    _ => {
                        return self.rt_err_kind("TypeError", "reduceSum axis must be integer");
                    }
                };
            ...
            "reduceMean" => {
                if dot_call.arguments.is_empty() {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.reduceMean(axis) requires at least 1 argument",
                    );
                }
                let ax_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let axis = match self.resolve(ax_ref) {
                    Some(ObjectData::Integer(n)) => {
                        ...
                        if a < 0 || a >= nd {
                            return self.rt_err_kind(
                                "TensorError",
                                format!("reduceMean axis {} out of range", n),
                            );
                        }
                        a as usize
                    }
                    _ => {
                        return self.rt_err_kind("TypeError", "reduceMean axis must be integer");
                    }
                };
                let keepdim = dot_call.arguments.len() > 1
                    && matches!(
            ...
            "reduceMax" => {
                if dot_call.arguments.is_empty() {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.reduceMax(axis) requires at least 1 argument",
                    );
                }
                let ax_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let axis = match self.resolve(ax_ref) {
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "reduceMax axis must be integer");
                    }
                };
                let keepdim = dot_call.arguments.len() > 1
                    && matches!(
            ...
            "maximum" => {
                ...
                let other_data = match self.resolve(other_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "maximum requires Tensor or scalar");
                    }
                };
                let new_data: Vec<f64> = data
                    .iter()
            ...
            "minimum" => {
                ...
                let other_data = match self.resolve(other_ref).cloned() {
                    ...
                    Some(ObjectData::Integer(v)) => vec![v as f64; data.len()],
                    _ => {
                        return self.rt_err_kind("TypeError", "minimum requires Tensor or scalar");
                    }
                };
                let new_data: Vec<f64> = data
                    .iter()
            ...
            "conv2d" => {
                if dot_call.arguments.len() != 4 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.conv2d(weights, bias, kernel, stride) requires 4 arguments",
                    );
                }
                if shape.len() != 4 {
                    return self.rt_err_kind(
                        "TypeError",
                        format!(
                            "Tensor.conv2d() input must be 4D [N, H, W, C_in], got {}D",
                            shape.len()
                        ),
                    );
                }
                ...
                let kernel = match self.resolve(k_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.conv2d() kernel must be a positive integer",
                        );
                    }
                };
                let stride = match self.resolve(s_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.conv2d() stride must be a positive integer",
                        );
                    }
                };
                let (w_shape, w_data) = match self.resolve(w_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.conv2d() weights must be a Tensor");
                    }
                };
                let b_data = match self.resolve(b_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.conv2d() bias must be a Tensor");
                    }
                };
                if w_shape.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.conv2d() weights must be 2D [kH*kW*C_in, C_out]",
                    );
                }
                ...
                if col_cols != kernel * kernel * c_in {
                    return self.rt_err_kind(
                        "TensorError",
                        format!(
                            "Tensor.conv2d() weights dim0={} != kernel*kernel*C_in={}",
                            col_cols,
                            kernel * kernel * c_in
                        ),
                    );
                ...
                if h < kernel || w_in < kernel {
                    return self.rt_err_kind(
                        "TensorError",
                        format!("Tensor.conv2d() spatial {}x{} < kernel {}", h, w_in, kernel),
                    );
                }
                            ...
                            for kh in 0..kernel {
                                for kw in 0..kernel {
                                    for ci in 0..c_in {
                                        let ih = oh * stride + kh;
                                        let iw = ow * stride + kw;
                                        let in_idx = nb * h * w_in * c_in
                                            + ih * w_in * c_in
                                            + iw * c_in
                                            + ci;
                                        let col_idx =
                                            row * col_cols + kh * kernel * c_in + kw * c_in + ci;
                ...
                for i in 0..col_rows {
                    for j in 0..c_out {
                        for l in 0..col_cols {
                            out_2d[i * c_out + j] +=
                                col_mat[i * col_cols + l] * w_data[l * c_out + j];
                        }
                    }
                }
                // broadcast add bias
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Conv2d {
                            in_id,
                            w_id,
                            b_id,
                            kernel,
            ...
            "max_pool2d" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.max_pool2d(kernel, stride) requires 2 arguments",
                    );
                }
                if shape.len() != 4 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.max_pool2d() input must be 4D [N, H, W, C]",
                    );
                }
                let k_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    EvalResult::Value(r) => r,
                ...
                let kernel = match self.resolve(k_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.max_pool2d() kernel must be a positive integer",
                        );
                    }
                };
                let stride = match self.resolve(s_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.max_pool2d() stride must be a positive integer",
                        );
                    }
                };
                ...
                if h < kernel || w_in < kernel {
                    return self.rt_err_kind(
                        "TensorError",
                        format!(
                            "Tensor.max_pool2d() spatial {}x{} < kernel {}",
                            h, w_in, kernel
                        ),
                    );
                }
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::MaxPool2d {
                            in_id,
                            kernel,
                            stride,
                            in_shape: shape,
            ...
            "layer_norm" => {
                if dot_call.arguments.len() != 3 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.layer_norm(gamma, beta, eps) requires 3 arguments",
                    );
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.layer_norm() input must be 2D [rows, cols]",
                    );
                }
                ...
                let gamma_data = match self.resolve(g_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.layer_norm() gamma must be a Tensor",
                        );
                    }
                };
                let beta_data = match self.resolve(b_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.layer_norm() beta must be a Tensor");
                    }
                };
                let eps = match self.resolve(e_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Tensor.layer_norm() eps must be a number");
                    }
                };
                let (rows, cols) = (shape[0], shape[1]);
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::LayerNorm {
                            in_id,
                            g_id,
                            b_id,
                            eps,
            ...
            "mha" => {
                if dot_call.arguments.len() != 5 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.mha(Wq, Wk, Wv, Wo, n_heads) requires 5 arguments",
                    );
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.mha() input must be 2D [seq_len, d_model]",
                    );
                }
                ...
                let n_heads = match self.resolve(nh_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.mha() n_heads must be a positive integer",
                        );
                    }
                };
                let (wq_shape, wq_data) = match self.resolve(wq_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.mha() Wq must be a Tensor");
                    }
                };
                ...
                for h in 0..n_heads {
                    ...
                    for row in 0..sl {
                        qh[row * dh..row * dh + dh]
                            .copy_from_slice(&q_proj[row * dm + h * dh..row * dm + h * dh + dh]);
                        kh[row * dh..row * dh + dh]
                            .copy_from_slice(&k_proj[row * dm + h * dh..row * dm + h * dh + dh]);
                        vh[row * dh..row * dh + dh]
                            .copy_from_slice(&v_proj[row * dm + h * dh..row * dm + h * dh + dh]);
                    }
                    ...
                    for i in 0..sl {
                        for j in 0..sl {
                            for k in 0..dh {
                                scores[i * sl + j] += qh[i * dh + k] * kh[j * dh + k];
                            }
                            scores[i * sl + j] *= scale;
                        }
                    }
                    ...
                    for row in 0..sl {
                        let mx = attn_h[row * sl..(row + 1) * sl]
                            .iter()
                            .cloned()
                            .fold(f64::NEG_INFINITY, f64::max);
                        let mut s = 0.0f64;
                        for c in 0..sl {
                            attn_h[row * sl + c] = (attn_h[row * sl + c] - mx).exp();
                            s += attn_h[row * sl + c];
                        }
                        for c in 0..sl {
                            attn_h[row * sl + c] /= s;
                    ...
                    for row in 0..sl {
                        concat_heads[row * dm + h * dh..row * dm + h * dh + dh]
                            .copy_from_slice(&head[row * dh..row * dh + dh]);
                    }
                }
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Mha {
                            x_id,
                            wq_id,
                            wk_id,
                            wv_id,
            ...
            "one_hot" => {
                if dot_call.arguments.len() != 1 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.one_hot(vocab_size) requires 1 argument",
                    );
                }
                if shape.len() != 1 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.one_hot() input must be 1D [seq_len] of indices",
                    );
                }
                ...
                let vocab = match self.resolve(v_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Tensor.one_hot() vocab_size must be a positive integer",
                        );
                    }
                };
                let seq_len = shape[0];
            ...
            "lstm" => {
                if dot_call.arguments.len() != 5 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.lstm(Wx, Wh, b, h0, c0) requires 5 arguments",
                    );
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.lstm() input must be 2D [seq_len, input_size]",
                    );
                }
                ...
                let (wx_shape, wx_data) = match self.resolve(wx_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.lstm() Wx must be a Tensor");
                    }
                };
                let (wh_shape, wh_data) = match self.resolve(wh_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.lstm() Wh must be a Tensor");
                    }
                };
                let b_data = match self.resolve(b_ref).cloned() {
                    Some(ObjectData::Tensor { data: d, .. }) => d,
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.lstm() b must be a Tensor");
                    }
                };
                let h0_data = match self.resolve(h0_ref).cloned() {
                    Some(ObjectData::Tensor { data: d, .. }) => d,
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.lstm() h0 must be a Tensor");
                    }
                };
                let c0_data = match self.resolve(c0_ref).cloned() {
                    Some(ObjectData::Tensor { data: d, .. }) => d,
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.lstm() c0 must be a Tensor");
                    }
                };
                if wx_shape.len() != 2 || wx_shape[0] != input_size || wx_shape[1] % 4 != 0 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.lstm() Wx must be [input_size, 4*hidden_size]",
                    );
                }
                ...
                if wh_shape.len() != 2 || wh_shape[0] != hidden_size || wh_shape[1] != four_h {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.lstm() Wh must be [hidden_size, 4*hidden_size]",
                    );
                }
                if b_data.len() != four_h {
                    return self.rt_err_kind(
                        "TensorError",
                        "Tensor.lstm() b must have length 4*hidden_size",
                    );
                }
                // Forward pass
                ...
                for j in 0..hidden_size.min(h0_data.len()) {
                    h_all[j] = h0_data[j];
                }
                for j in 0..hidden_size.min(c0_data.len()) {
                    c_all[j] = c0_data[j];
                }
                for t in 0..seq_len {
                    ...
                    for j in 0..four_h {
                        for k in 0..input_size {
                            z[j] += x_t[k] * wx_data[k * four_h + j];
                        }
                        for k in 0..hidden_size {
                            z[j] += h_prev[k] * wh_data[k * four_h + j];
                        }
                    }
                    let sig = |x: f64| 1.0f64 / (1.0 + (-x).exp());
                    let i_t: Vec<f64> = z[..hidden_size].iter().map(|&x| sig(x)).collect();
                    let f_t: Vec<f64> = z[hidden_size..2 * hidden_size]
                        .iter()
                        .map(|&x| sig(x))
                        .collect();
                    let g_t: Vec<f64> = z[2 * hidden_size..3 * hidden_size]
                        .iter()
                        .map(|&x| x.tanh())
                        .collect();
                    let o_t: Vec<f64> = z[3 * hidden_size..].iter().map(|&x| sig(x)).collect();
                    let c_t: Vec<f64> = (0..hidden_size)
                        .map(|j| f_t[j] * c_prev[j] + i_t[j] * g_t[j])
                        .collect();
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Lstm {
                            x_id,
                            wx_id,
                            wh_id,
                            b_id,
            ...
            "gru" => {
                if dot_call.arguments.len() != 4 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.gru(Wx, Wh, b, h0) requires 4 arguments",
                    );
                }
                if shape.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.gru() input must be 2D [seq_len, input_size]",
                    );
                }
                let (seq_len, input_size) = (shape[0], shape[1]);
                let wx_ref = match self.eval_expression(&dot_call.arguments[0]) {
                ...
                let (wx_shape, wx_data) = match self.resolve(wx_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.gru() Wx must be a Tensor");
                    }
                };
                let (wh_shape, wh_data) = match self.resolve(wh_ref).cloned() {
                    Some(ObjectData::Tensor {
                    ...
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.gru() Wh must be a Tensor");
                    }
                };
                let b_data = match self.resolve(b_ref).cloned() {
                    Some(ObjectData::Tensor { data: d, .. }) => d,
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.gru() b must be a Tensor");
                    }
                };
                let h0_data = match self.resolve(h0_ref).cloned() {
                    Some(ObjectData::Tensor { data: d, .. }) => d,
                    _ => {
                        return self.rt_err_kind("TypeError", "Tensor.gru() h0 must be a Tensor");
                    }
                };
                if wx_shape.len() != 2 || wx_shape[0] != input_size || wx_shape[1] % 3 != 0 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.gru() Wx must be [input_size, 3*hidden_size]",
                    );
                }
                ...
                if wh_shape.len() != 2 || wh_shape[0] != hidden_size || wh_shape[1] != three_h {
                    return self.rt_err_kind(
                        "TypeError",
                        "Tensor.gru() Wh must be [hidden_size, 3*hidden_size]",
                    );
                }
                if b_data.len() != three_h {
                    return self.rt_err_kind(
                        "TensorError",
                        "Tensor.gru() b must have length 3*hidden_size",
                    );
                }
                let mut h_all = vec![0.0f64; (seq_len + 1) * hidden_size];
                ...
                for t in 0..seq_len {
                    ...
                    for j in 0..2 * hidden_size {
                        for k in 0..input_size {
                            rz_pre[j] += x_t[k] * wx_data[k * three_h + j];
                        }
                        for k in 0..hidden_size {
                            rz_pre[j] += h_prev[k] * wh_data[k * three_h + j];
                        }
                    }
                    ...
                    for j in 0..hidden_size {
                        for k in 0..input_size {
                            n_pre[j] += x_t[k] * wx_data[k * three_h + 2 * hidden_size + j];
                        }
                        for k in 0..hidden_size {
                            n_pre[j] +=
                                r_t[k] * h_prev[k] * wh_data[k * three_h + 2 * hidden_size + j];
                        }
                    }
                    let n_t: Vec<f64> = n_pre.iter().map(|&x| x.tanh()).collect();
                    let h_t: Vec<f64> = (0..hidden_size)
                        .map(|j| (1.0 - z_t[j]) * h_prev[j] + z_t[j] * n_t[j])
                        .collect();
                ...
                if self.ad_recording {
                    ...
                    self.ad_push(
                        out_ref,
                        crate::evaluator::namespaces_autodiff::TapeOp::Gru {
                            x_id,
                            wx_id,
                            wh_id,
                            b_id,
            ...
            "outer" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Tensor.outer(other) requires 1 argument");
                }
                if shape.len() != 1 {
                ...
                match self.resolve(other_ref).cloned() {
                    Some(ObjectData::Tensor {
                        ...
                        if os.len() != 1 {
                            return self.rt_err_kind(
                                "TypeError",
                                "Tensor.outer() argument must be a 1D Tensor",
                            );
                        }
                        let m = shape[0];
                        let n = os[0];
            ...
            _ => self.rt_err_kind(
                "TypeError",
                format!("Unknown Tensor method '{}'", dot_call.method),
            ),
        }
    }
    ...
    fn checked_tensor_element_count(&mut self, shape: &[usize]) -> Result<usize, EvalResult> {
            ...
            .try_fold(1usize, |total, dimension| total.checked_mul(*dimension))
        {
            ...
            None => {
                return Err(self.fatal_err_kind(
                    "ResourceError",
                    "Tensor shape element count overflows the platform size",
                ));
            }
        };
        if total > MAX_TENSOR_ELEMENTS {
            return Err(self.fatal_err_kind(
                "ResourceError",
                format!("Tensor size {total} elements exceeds maximum ({MAX_TENSOR_ELEMENTS})"),
            ));
        }
        Ok(total)
    }

    pub(super) fn eval_shape_expr(
        &mut self,
        expr: &ast::Expression,
                    ...
                    match e {
                        ...
                        OwnedValue::Integer(n) => {
                            return Err(self.rt_err_kind(
                                "RangeError",
                                format!("Tensor shape dimensions must be positive, got {n}"),
                            ));
                        }
                        _ => {
                            return Err(self.rt_err_kind(
                                "TypeError",
                                "Tensor shape must be an array of positive integers",
                            ));
                        }
                    }
                }
                if shape.is_empty() {
                    return Err(self.rt_err_kind("RangeError", "Tensor shape cannot be empty"));
                }
                self.checked_tensor_element_count(&shape)?;
                Ok(shape)
            }
            _ => Err(self.rt_err_kind("TypeError", "Tensor shape must be an array like [2, 3]")),
        }
    }
                        ...
                        match row {
                            OwnedValue::Array { elements: cols, .. } => {
                                if row_i == 0 {
                                    ...
                                    return self.rt_err_kind(
                                        "TensorError",
                                        "Tensor.from() — all rows must have the same length",
                                    );
                                }
                                for c in cols {
                                    match c {
                                        ...
                                        _ => {
                                            return self.rt_err_kind(
                                                "TypeError",
                                                "Tensor.from() — elements must be numbers",
                                            );
                                        }
                                    }
                                }
                            ...
                            _ => {
                                return self.rt_err_kind(
                                    "TensorError",
                                    "Tensor.from() — mixed nesting not allowed",
                                );
                            }
                        }
                    }
        ...
        let result_data = match self.resolve(arg_ref).cloned() {
            Some(ObjectData::Tensor {
                ...
                if s2 != shape {
                    return self.rt_err_kind(
                        "TensorError",
                        format!("Tensor.{}() shape mismatch: {:?} vs {:?}", op, shape, s2),
                    );
                }
                data.iter()
                    .zip(d2.iter())
            ...
            Some(ObjectData::Decimal(d)) => data
                .iter()
                .map(|a| match op {
                    "add" => a + d,
                    "sub" => a - d,
            ...
            _ => {
                return self.rt_err_kind(
                    "TypeError",
                    format!("Tensor.{}() requires a Tensor or numeric argument", op),
                );
            }
        };
        let out_ref = self.alloc(ObjectData::Tensor {
    ...
    pub(super) fn broadcast_shape(a: &[usize], b: &[usize]) -> Option<Vec<usize>> {
        ...
        for i in 0..ndim {
            let ai = if i < ndim - a.len() {
                1
            } else {
                a[i - (ndim - a.len())]
            };
            let bi = if i < ndim - b.len() {
                1
            } else {
                b[i - (ndim - b.len())]
            };
    ...
    pub(super) fn broadcast_data(
        data: &[f64],
        from_shape: &[usize],
        to_shape: &[usize],
    ) -> Option<Vec<f64>> {
        let ndim = to_shape.len();
        if from_shape.len() > ndim {
            return None;
        }
        ...
        let padded: Vec<usize> = (0..pad)
            .map(|_| 1)
            .chain(from_shape.iter().cloned())
            .collect();
        // Verify compatible
        for i in 0..ndim {
            if padded[i] != to_shape[i] && padded[i] != 1 {
                return None;
            }
        }
    ...
    pub(super) fn reduce_along_axis(
        ...
        let out_shape_no_ax: Vec<usize> = shape
            .iter()
            .cloned()
            .enumerate()
        ...
        for flat_in in 0..data.len() {
            ...
            for i in 0..ndim {
                ...
                if i == axis {
                    ...
                    if ax_no < out_strides_no_ax.len() {
                        out_flat += idx * out_strides_no_ax[ax_no];
                    }
                }
            }
            let _ = out_ax;
