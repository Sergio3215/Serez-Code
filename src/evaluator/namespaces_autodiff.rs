...
#[cfg(test)]
mod weights_file_tests {
    use crate::evaluator::Evaluator;

    fn header(count: u32) -> Vec<u8> {
        let mut bytes = b"SZWT".to_vec();
        bytes.push(1);
    ...
    #[test]
    fn corrupt_weights_reject_huge_tensor_count_before_allocation() {
        let bytes = header(u32::MAX);
        let error = Evaluator::parse_weights_bytes(&bytes, "memory.szw").unwrap_err();
        assert!(error.contains("tensor count"));
    }

    #[test]
    fn corrupt_weights_reject_excessive_rank() {
        let mut bytes = header(1);
        bytes.push(255);
        let error = Evaluator::parse_weights_bytes(&bytes, "memory.szw").unwrap_err();
        assert!(error.contains("tensor rank"));
    }

    #[test]
    fn corrupt_weights_reject_overflowing_data_length() {
        let mut bytes = header(1);
        bytes.push(0);
        bytes.extend_from_slice(&u64::MAX.to_le_bytes());
        let error = Evaluator::parse_weights_bytes(&bytes, "memory.szw").unwrap_err();
        assert!(error.contains("too large") || error.contains("exceeds limit"));
    }

    #[test]
    fn corrupt_weights_reject_shape_length_mismatch() {
        let mut bytes = header(1);
        bytes.push(1);
        ...
        let error = Evaluator::parse_weights_bytes(&bytes, "memory.szw").unwrap_err();
        assert!(error.contains("does not match data length"));
    }
}
...
impl super::Evaluator {
        ...
        match dot_call.method.as_str() {
            ...
            "backward" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Autodiff.backward(loss) requires 1 argument");
                }
                ...
                let (loss_data, loss_tid) = match self.resolve(loss_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.backward() argument must be a Tensor",
                        );
                    }
                };
                let seed_id = match self.autodiff.tensor_ids.get(&loss_tid).copied() {
                    ...
                    None => {
                        return self.rt_err_kind(
                            "AutodiffError",
                            "Autodiff.backward() loss tensor was not created during tape recording",
                        );
                    }
                };
                self.autodiff
                    ...
                    match &entry.op {
                        ...
                        TapeOp::Conv2d {
                            ...
                            for l in 0..cc {
                                for j in 0..co {
                                    for i in 0..cr {
                                        dw[l * co + j] +=
                                            col_mat[i * cc + l] * out_grad[i * co + j];
                                    }
                                }
                            }
                        ...
                        TapeOp::Lstm {
                            ...
                            for t in (0..sl).rev() {
                                ...
                                // do_t = dh * tanh(c_t)
                                let do_t: Vec<f64> =
                                    (0..hs).map(|j| dh[j] * c_t[j].tanh()).collect();
                                ...
                                for k in 0..is {
                                    for j in 0..four_h {
                                        dwx[k * four_h + j] += x_t[k] * dz[j];
                                    }
                                }
                                // dWh += h_prev^T ⊗ dz
                                for k in 0..hs {
                                    for j in 0..four_h {
                                        dwh[k * four_h + j] += h_prev[k] * dz[j];
                                    }
                                }
                                for j in 0..four_h {
                                    db[j] += dz[j];
                        ...
                        TapeOp::LayerNorm {
                            ...
                            for row in 0..r {
                                ...
                                let d_xnorm: Vec<f64> =
                                    (0..c).map(|j| g[j] * gamma_data[j]).collect();
                                let sum_d_xnorm: f64 = d_xnorm.iter().sum();
                                let sum_d_xnorm_xn: f64 =
                                    d_xnorm.iter().zip(xn.iter()).map(|(a, b)| a * b).sum();
                                for col in 0..c {
                                    dx[row * c + col] = (d_xnorm[col]
                                        - sum_d_xnorm / n
                                        - xm[col] / (std_i * std_i) * sum_d_xnorm_xn / n)
                                        / std_i;
                                }
                            }
                            Self::accum_grad(&mut self.autodiff.grads, *in_id, dx);
                        ...
                        TapeOp::Mha {
                            ...
                            // Helper: [m,k] @ [k,n] → [m,n]
                            let mm =
                                |a: &[f64], m: usize, k: usize, b: &[f64], n: usize| -> Vec<f64> {
                            ...
                            for i in 0..sl {
                                for k in 0..dm {
                                    for j in 0..dm {
                                        d_concat2[i * dm + k] +=
                                            out_grad[i * dm + j] * wo_data[k * dm + j];
                                    }
                                }
                            }
                            ...
                            for k in 0..dm {
                                for j in 0..dm {
                                    for i in 0..sl {
                                        dwo[k * dm + j] +=
                                            concat_heads[i * dm + k] * out_grad[i * dm + j];
                                    }
                                }
                            }
                            ...
                            for h in 0..nh {
                                ...
                                for row in 0..sl {
                                    kh[row * d..row * d + d].copy_from_slice(
                                        &k_proj[row * dm + h * d..row * dm + h * d + d],
                                    );
                                    qh[row * d..row * d + d].copy_from_slice(
                                        &q_proj[row * dm + h * d..row * dm + h * d + d],
                                    );
                                    vh[row * d..row * d + d].copy_from_slice(
                                        &v_proj[row * dm + h * d..row * dm + h * d + d],
                                    );
                                }
                                // d_head_h = d_concat[:, h*d:(h+1)*d]  [sl, dh]
                                let mut dhead = vec![0.0f64; sl * d];
                                for row in 0..sl {
                                    dhead[row * d..row * d + d].copy_from_slice(
                                        &d_concat2[row * dm + h * d..row * dm + h * d + d],
                                    );
                                }
                                ...
                                for i in 0..sl {
                                    for j in 0..sl {
                                        for k in 0..d {
                                            dattn[i * sl + j] += dhead[i * d + k] * vh[j * d + k];
                                        }
                                    }
                                }
                                ...
                                for j in 0..sl {
                                    for k in 0..d {
                                        for i in 0..sl {
                                            dvh[j * d + k] += attn_h[i * sl + j] * dhead[i * d + k];
                                        }
                                    }
                                }
                                ...
                                for row in 0..sl {
                                    ...
                                    let dot: f64 =
                                        g.iter().zip(s.iter()).map(|(gi, si)| gi * si).sum();
                                    for col in 0..sl {
                                        dscores[row * sl + col] = s[col] * (g[col] - dot) * scale;
                                    }
                                }
                                ...
                                for j in 0..sl {
                                    for k in 0..d {
                                        for i in 0..sl {
                                            dkh[j * d + k] += dscores[i * sl + j] * qh[i * d + k];
                                        }
                                    }
                                }
                        ...
                        TapeOp::Gelu {
                            ...
                            let g: Vec<f64> = out_grad
                                ...
                                .map(|(dout, &x)| {
                                    ...
                                    let sech2 = 1.0 - t * t;
                                    let d = 0.5 * (1.0 + t)
                                        + 0.5 * x * sech2 * c * (1.0 + 3.0 * 0.044715 * x * x);
                                    dout * d
                                })
                                .collect();
                            Self::accum_grad(&mut self.autodiff.grads, *in_id, g);
                        ...
                        TapeOp::BatchNorm {
                            ...
                            for j in 0..c {
                                ...
                                for i in 0..n {
                                    let d_xn = out_grad[i * c + j] * gamma_data[j];
                                    dx[i * c + j] =
                                        (d_xn - sum_dxn / nf - x_norm[i * c + j] * sum_dxn_xn / nf)
                                            / std_j;
                                }
                        ...
                        TapeOp::Gru {
                            ...
                            for t in (0..sl).rev() {
                                ...
                                let dz_gate: Vec<f64> =
                                    (0..hs).map(|j| dh[j] * (n_t[j] - h_prev[j])).collect();
                                let mut dh_prev: Vec<f64> =
                                    (0..hs).map(|j| dh[j] * (1.0 - z_t[j])).collect();
                                // n_t = tanh(x@Wx_n + (r*h_prev)@Wh_n + b_n)
                                let dz_n: Vec<f64> =
                                    (0..hs).map(|j| dn[j] * (1.0 - n_t[j] * n_t[j])).collect();
                                ...
                                for j in 0..hs {
                                    dh_prev[j] += d_rh[j] * r_t[j];
                                }
                                // r_t sigmoid deriv
                                let dz_r: Vec<f64> =
                                    (0..hs).map(|j| dr[j] * r_t[j] * (1.0 - r_t[j])).collect();
                                // z_t sigmoid deriv
                                let dz_z: Vec<f64> = (0..hs)
                                    .map(|j| dz_gate[j] * z_t[j] * (1.0 - z_t[j]))
                                    .collect();
                                ...
                                for k in 0..hs {
                                    for j in 0..2 * hs {
                                        dwh[k * three_h + j] += h_prev[k] * dz[j];
                                    }
                                    for j in 0..hs {
                                        dwh[k * three_h + 2 * hs + j] +=
                                            r_t[k] * h_prev[k] * dz_n[j];
                                    }
                                }
                                for j in 0..three_h {
                                    db[j] += dz[j];
            ...
            "gradient" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Autodiff.gradient(tensor) requires 1 argument");
                }
                ...
                let (shape, t_tid) = match self.resolve(t_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.gradient() argument must be a Tensor",
                        );
                    }
                };
                let tape_id = match self.autodiff.tensor_ids.get(&t_tid).copied() {
                    ...
                    None => {
                        return self.rt_err_kind(
                            "AutodiffError",
                            format!(
                                "Autodiff.gradient() tensor was not recorded during tape (tid={})",
                                t_tid
                            ),
                        );
                    }
                ...
                Ok(ExecutionFlow::Value(self.alloc_tensor(shape, grad_data)))
            }
            ...
            "mseLoss" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.mseLoss(pred, target) requires 2 arguments",
                    );
                }
                let pred_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let tgt_ref = match self.eval_expression(&dot_call.arguments[1]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let (pred_data, _pred_shape, pred_tid) = match self.resolve(pred_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.mseLoss pred must be a Tensor");
                    }
                };
                let target_data = match self.resolve(tgt_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.mseLoss target must be a Tensor");
                    }
                };
                if pred_data.len() != target_data.len() {
                    return self.rt_err_kind(
                        "AutodiffError",
                        "Autodiff.mseLoss pred and target must have same size",
                    );
                }
                let n = pred_data.len();
                let loss: f64 = pred_data
                    .iter()
                    .zip(target_data.iter())
                    .map(|(p, t)| (p - t).powi(2))
                    .sum::<f64>()
            ...
            "maeLoss" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.maeLoss(pred, target) requires 2 arguments",
                    );
                }
                let pred_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                ...
                let (pred_data, _pred_shape, pred_tid) = match self.resolve(pred_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.maeLoss pred must be a Tensor");
                    }
                };
                let target_data = match self.resolve(tgt_ref).cloned() {
                    Some(ObjectData::Tensor { data, .. }) => data,
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.maeLoss target must be a Tensor");
                    }
                };
                let n = pred_data.len();
                let loss: f64 = pred_data
            ...
            "bceLoss" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.bceLoss(pred, target) requires 2 arguments",
                    );
                }
                let pred_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                ...
                let (pred_data, _ps, pred_tid) = match self.resolve(pred_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.bceLoss pred must be a Tensor");
                    }
                };
                let target_data = match self.resolve(tgt_ref).cloned() {
                    Some(ObjectData::Tensor { data, .. }) => data,
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.bceLoss target must be a Tensor");
                    }
                };
                let n = pred_data.len();
                let eps = 1e-12_f64;
            ...
            "crossEntropyLoss" => {
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.crossEntropyLoss(logits, targets) requires 2 arguments",
                    );
                }
                let logits_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let tgt_ref = match self.eval_expression(&dot_call.arguments[1]) {
                ...
                let (logits_data, logits_shape, logits_tid) =
                    match self.resolve(logits_ref).cloned() {
                        Some(ObjectData::Tensor { data, shape, tid }) => (data, shape, tid),
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Autodiff.crossEntropyLoss logits must be a Tensor",
                            );
                        }
                    };
                ...
                let target_data = match self.resolve(tgt_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.crossEntropyLoss targets must be a Tensor or Array",
                        );
                    }
                };
                let (batch, classes) = if logits_shape.len() == 2 {
                    ...
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.crossEntropyLoss logits must be 2D [batch, classes]",
                    );
                };
                if target_data.len() != batch {
                    return self.rt_err_kind(
                        "AutodiffError",
                        format!(
                            "Autodiff.crossEntropyLoss targets length {} != batch {}",
                            target_data.len(),
                            batch
                        ),
                    );
                ...
                for b in 0..batch {
                    ...
                    for c in 0..classes {
                        probs[b * classes + c] = exps[c] / sum_exp;
                    }
                }
                let eps = 1e-12_f64;
                let target_indices: Vec<usize> =
                    target_data.iter().map(|&t| t.round() as usize).collect();
            ...
            "clipGrad" => {
                if dot_call.arguments.len() < 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.clipGrad(grad, max_norm) requires 2 arguments",
                    );
                }
                let grad_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let max_norm_ref = match self.eval_expression(&dot_call.arguments[1]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let (grad_data, grad_shape) = match self.resolve(grad_ref).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.clipGrad grad must be a Tensor");
                    }
                };
                let max_norm = match self.resolve(max_norm_ref) {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.clipGrad max_norm must be a number",
                        );
                    }
                };
                ...
                Ok(ExecutionFlow::Value(self.alloc_tensor(grad_shape, clipped)))
            }
            ...
            "clipGradNorm" => {
                ...
                if dot_call.arguments.len() < 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.clipGradNorm(grads, max_norm) requires 2 arguments",
                    );
                }
                let grads_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let max_norm_r = match self.eval_expression(&dot_call.arguments[1]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let max_norm = match self.resolve(max_norm_r) {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.clipGradNorm max_norm must be a number",
                        );
                    }
                };
                let elements = match self.resolve(grads_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.clipGradNorm grads must be an Array of Tensors",
                        );
                    }
                };
                // Compute global norm
                ...
                for elem in &elements {
                    match elem {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Autodiff.clipGradNorm all elements must be Tensors",
                            );
                        }
                    }
                }
                ...
                for (data, shape) in grads_data {
                    ...
                    result_owned.push(crate::region::OwnedValue::Tensor {
                        shape,
                        data: clipped,
                        tid: 0,
                    });
                }
                let arr = ObjectData::Array {
                    element_type: None,
                    elements: result_owned,
                };
                Ok(ExecutionFlow::Value(self.alloc(arr)))
            }
            ...
            "xavierUniform" => {
                if dot_call.arguments.is_empty() {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.xavierUniform([shape]) requires shape argument",
                    );
                }
                ...
                let data: Vec<f64> = (0..total)
                    ...
                    .collect();
                Ok(ExecutionFlow::Value(self.alloc_tensor(shape, data)))
            }
            ...
            "xavierNormal" => {
                if dot_call.arguments.is_empty() {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.xavierNormal([shape]) requires shape argument",
                    );
                }
                let shape = match self.eval_shape_expr(&dot_call.arguments[0]) {
                    Ok(s) => s,
            ...
            "heUniform" => {
                if dot_call.arguments.is_empty() {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.heUniform([shape]) requires shape argument",
                    );
                }
                let shape = match self.eval_shape_expr(&dot_call.arguments[0]) {
                    Ok(s) => s,
            ...
            "heNormal" => {
                if dot_call.arguments.is_empty() {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.heNormal([shape]) requires shape argument",
                    );
                }
                let shape = match self.eval_shape_expr(&dot_call.arguments[0]) {
                    Ok(s) => s,
            ...
            "adamStep" => {
                ...
                if dot_call.arguments.len() < 6 {
                    return self.rt_err_kind(
                        "AutodiffError",
                        "Autodiff.adamStep(param, grad, m, v, step, lr, [beta1, beta2, eps])",
                    );
                }
                let mut args = Vec::new();
                for arg in &dot_call.arguments {
                    match self.eval_expression(arg) {
                        Ok(ExecutionFlow::Value(r)) => args.push(r),
                        other => return other,
                    }
                }
                let (param, p_shape) = match self.resolve(args[0]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamStep param must be a Tensor");
                    }
                };
                let (grad, _) = match self.resolve(args[1]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamStep grad must be a Tensor");
                    }
                };
                let (m_data, _) = match self.resolve(args[2]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamStep m must be a Tensor");
                    }
                };
                let (v_data, _) = match self.resolve(args[3]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamStep v must be a Tensor");
                    }
                };
                let step = match self.resolve(args[4]) {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamStep step must be a number");
                    }
                };
                let lr = match self.resolve(args[5]) {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamStep lr must be a number");
                    }
                };
                let beta1 = if args.len() > 6 {
                    match self.resolve(args[6]) {
                        Some(ObjectData::Decimal(d)) => *d,
                        Some(ObjectData::Integer(n)) => *n as f64,
                        _ => 0.9,
                    }
                ...
                let eps = if args.len() > 8 {
                    match self.resolve(args[8]) {
                        Some(ObjectData::Decimal(d)) => *d,
                        Some(ObjectData::Integer(n)) => *n as f64,
                        _ => 1e-8,
                    }
            ...
            "adamwStep" => {
                ...
                if dot_call.arguments.len() < 6 {
                    return self.rt_err_kind(
                        "AutodiffError",
                        "Autodiff.adamwStep(param, grad, m, v, step, lr, [wd, beta1, beta2, eps])",
                    );
                }
                let mut args = Vec::new();
                for arg in &dot_call.arguments {
                ...
                let (param, p_shape) = match self.resolve(args[0]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamwStep param must be Tensor");
                    }
                };
                let (grad, _) = match self.resolve(args[1]).cloned() {
                    Some(ObjectData::Tensor { data, shape, .. }) => (data, shape),
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamwStep grad must be Tensor");
                    }
                };
                let (m_data, _) = match self.resolve(args[2]).cloned() {
                    Some(ObjectData::Tensor { data, shape, .. }) => (data, shape),
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamwStep m must be Tensor");
                    }
                };
                let (v_data, _) = match self.resolve(args[3]).cloned() {
                    Some(ObjectData::Tensor { data, shape, .. }) => (data, shape),
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.adamwStep v must be Tensor");
                    }
                };
                let step = match self.resolve(args[4]) {
                    Some(ObjectData::Integer(n)) => *n as f64,
                    Some(ObjectData::Decimal(d)) => *d,
                    _ => {
                        return self.rt_err_kind("TypeError", "step must be number");
                    }
                };
                let lr = match self.resolve(args[5]) {
                    Some(ObjectData::Decimal(d)) => *d,
                    Some(ObjectData::Integer(n)) => *n as f64,
                    _ => {
                        return self.rt_err_kind("TypeError", "lr must be number");
                    }
                };
                let wd = if args.len() > 6 {
                    match self.resolve(args[6]) {
                        Some(ObjectData::Decimal(d)) => *d,
                        Some(ObjectData::Integer(n)) => *n as f64,
                        _ => 0.01,
                ...
                let eps = if args.len() > 9 {
                    match self.resolve(args[9]) {
                        Some(ObjectData::Decimal(d)) => *d,
                        _ => 1e-8,
                    }
                } else {
            ...
            "sgdStep" => {
                ...
                if dot_call.arguments.len() < 4 {
                    return self.rt_err_kind(
                        "AutodiffError",
                        "Autodiff.sgdStep(param, grad, velocity, lr, [momentum, weight_decay])",
                    );
                }
                let mut args = Vec::new();
                for arg in &dot_call.arguments {
                ...
                let (param, p_shape) = match self.resolve(args[0]).cloned() {
                    Some(ObjectData::Tensor { data, shape, .. }) => (data, shape),
                    _ => {
                        return self.rt_err_kind("TypeError", "param must be Tensor");
                    }
                };
                let (mut grad, _) = match self.resolve(args[1]).cloned() {
                    Some(ObjectData::Tensor { data, shape, .. }) => (data, shape),
                    _ => {
                        return self.rt_err_kind("TypeError", "grad must be Tensor");
                    }
                };
                let (vel, _) = match self.resolve(args[2]).cloned() {
                    Some(ObjectData::Tensor { data, shape, .. }) => (data, shape),
                    _ => {
                        return self.rt_err_kind("TypeError", "velocity must be Tensor");
                    }
                };
                let lr = match self.resolve(args[3]) {
                    Some(ObjectData::Decimal(d)) => *d,
                    Some(ObjectData::Integer(n)) => *n as f64,
                    _ => {
                        return self.rt_err_kind("TypeError", "lr must be number");
                ...
                let wd = if args.len() > 5 {
                    match self.resolve(args[5]) {
                        Some(ObjectData::Decimal(d)) => *d,
                        _ => 0.0,
                    }
                } else {
            ...
            "rmspropStep" => {
                ...
                if dot_call.arguments.len() < 4 {
                    return self.rt_err_kind(
                        "AutodiffError",
                        "Autodiff.rmspropStep(param, grad, sq_avg, lr, [alpha, eps])",
                    );
                }
                let mut args = Vec::new();
                for arg in &dot_call.arguments {
                ...
                let (sq_avg, _) = match self.resolve(args[2]).cloned() {
                    Some(ObjectData::Tensor { data, shape, .. }) => (data, shape),
                    _ => {
                        return self.rt_err_kind("TypeError", "sq_avg must be Tensor");
                    }
                };
                let lr = match self.resolve(args[3]) {
                    Some(ObjectData::Decimal(d)) => *d,
            ...
            "batchNorm" => {
                ...
                if dot_call.arguments.len() < 4 {
                    return self.rt_err_kind(
                        "AutodiffError",
                        "Autodiff.batchNorm(x, gamma, beta, training, [eps])",
                    );
                }
                let mut args = Vec::new();
                for arg in &dot_call.arguments {
                ...
                let (x_data, x_shape, x_tid) = match self.resolve(args[0]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.batchNorm x must be a Tensor");
                    }
                };
                let (g_data, g_tid) = match self.resolve(args[1]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.batchNorm gamma must be a Tensor");
                    }
                };
                let (b_data, b_tid) = match self.resolve(args[2]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.batchNorm beta must be a Tensor");
                    }
                };
            ...
            "dropout" => {
                ...
                if dot_call.arguments.len() < 2 {
                    return self
                        .rt_err_kind("AutodiffError", "Autodiff.dropout(x, p, [training=true])");
                }
                let mut args = Vec::new();
                for arg in &dot_call.arguments {
                    match self.eval_expression(arg) {
                ...
                let (x_data, x_shape, x_tid) = match self.resolve(args[0]).cloned() {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.dropout x must be a Tensor");
                    }
                };
                let p = match self.resolve(args[1]) {
                    ...
                    _ => {
                        return self
                            .rt_err_kind("TypeError", "Autodiff.dropout p must be a number");
                    }
                };
                let training = if args.len() > 2 {
                    match self.resolve(args[2]) {
                        Some(ObjectData::Boolean(b)) => *b,
                        _ => true,
                    }
                } else {
                ...
                if !training || p <= 0.0 {
                    return Ok(ExecutionFlow::Value(self.alloc_tensor(x_shape, x_data)));
                }
                let keep_prob = 1.0 - p;
                let n = x_data.len();
                let mask: Vec<f64> = (0..n)
                    .map(|_| if self.lcg_next_f64() >= p { 1.0 } else { 0.0 })
                    .collect();
                let out_data: Vec<f64> = x_data
                    .iter()
                    .zip(mask.iter())
            ...
            "embedding" => {
                ...
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.embedding(indices, weight) requires 2 arguments",
                    );
                }
                let idx_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let w_ref = match self.eval_expression(&dot_call.arguments[1]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let (w_data, w_shape, w_tid) = match self.resolve(w_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.embedding weight must be a Tensor [vocab, emb_dim]",
                        );
                    }
                };
                if w_shape.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.embedding weight must be 2D [vocab_size, emb_dim]",
                    );
                }
                ...
                let indices: Vec<usize> = match self.resolve(idx_ref).cloned() {
                    Some(ObjectData::Tensor { data, .. }) => data
                        .iter()
                        .map(|&x| (x.round() as i64).max(0) as usize)
                        .collect(),
                    Some(ObjectData::Array { elements, .. }) => elements
                        .iter()
                        .filter_map(|elem| match elem {
                            crate::region::OwnedValue::Integer(v) => Some((*v).max(0) as usize),
                            crate::region::OwnedValue::Decimal(d) => {
                                Some((d.round() as i64).max(0) as usize)
                            }
                            _ => None,
                        })
                        .collect(),
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.embedding indices must be a Tensor or Array",
                        );
                    }
                };
            ...
            "saveWeights" => {
                ...
                if dot_call.arguments.len() != 2 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.saveWeights(path, tensors) requires 2 arguments",
                    );
                }
                ...
                let arr_ref = match self.eval_expression(&dot_call.arguments[1]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                };
                let path = match self.resolve(path_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.saveWeights path must be a string",
                        );
                    }
                };
                let elements = match self.resolve(arr_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.saveWeights tensors must be an Array",
                        );
                    }
                };
                // Collect tensor data
                ...
                for elem in &elements {
                    match elem {
                        ...
                        _ => {
                            return self.rt_err_kind(
                                "TypeError",
                                "Autodiff.saveWeights all elements must be Tensors",
                            );
                        }
                    }
                }
                ...
                match Self::write_weights_file(&path, &tensors) {
                    ...
                    Err(e) => self.rt_err_kind(
                        "AutodiffError",
                        format!("Autodiff.saveWeights failed: {}", e),
                    ),
                }
            }
            ...
            "loadWeights" => {
                ...
                if dot_call.arguments.len() != 1 {
                    return self.rt_err_kind(
                        "TypeError",
                        "Autodiff.loadWeights(path) requires 1 argument",
                    );
                }
                let path_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                ...
                let path = match self.resolve(path_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.loadWeights path must be a string",
                        );
                    }
                };
                match Self::read_weights_file(&path) {
                    Ok(tensors) => {
                        let n = tensors.len();
                        let owned: Vec<crate::region::OwnedValue> = tensors
                            .into_iter()
                            .map(|(shape, data)| crate::region::OwnedValue::Tensor {
                                shape,
                                data,
                                tid: 0,
                            })
                    ...
                    Err(e) => self.rt_err_kind(
                        "AutodiffError",
                        format!("Autodiff.loadWeights failed: {}", e),
                    ),
                }
            }
            ...
            "stopGrad" | "detach" => {
                if dot_call.arguments.len() != 1 {
                    return self
                        .rt_err_kind("TypeError", "Autodiff.stopGrad(tensor) requires 1 argument");
                }
                let t_ref = match self.eval_expression(&dot_call.arguments[0]) {
                    Ok(ExecutionFlow::Value(r)) => r,
                    other => return other,
                ...
                let (shape, data) = match self.resolve(t_ref).cloned() {
                    ...
                    _ => {
                        return self.rt_err_kind(
                            "TypeError",
                            "Autodiff.stopGrad() argument must be a Tensor",
                        );
                    }
                };
            ...
            _ => self.rt_err_kind(
                "TypeError",
                format!("Unknown Autodiff method '{}'", dot_call.method),
            ),
        }
    }
    ...
    fn parse_weights_bytes(
        buf: &[u8],
        source: &str,
    ) -> Result<Vec<(Vec<usize>, Vec<f64>)>, String> {
        const MAX_TENSORS: usize = 100_000;
        const MAX_RANK: usize = 64;
        const MAX_TOTAL_VALUES: usize = 10_000_000;
        if buf.len() < 9 {
            return Err(format!("'{}' is too short to be a valid .szw file", source));
        }
        if &buf[..4] != b"SZWT" {
            return Err(format!("'{}' is not a valid .szw file (bad magic)", source));
        }
        let version = buf[4];
        if version != 1 {
        ...
        for _ in 0..count {
            if pos >= buf.len() {
                return Err("unexpected end of file reading tensor header".to_string());
            }
            let ndim = buf[pos] as usize;
            pos += 1;
            if ndim > MAX_RANK {
                return Err(format!("tensor rank {} exceeds limit {}", ndim, MAX_RANK));
            }
            let mut shape = Vec::with_capacity(ndim);
            for _ in 0..ndim {
                let end = pos
                    .checked_add(8)
                    .ok_or_else(|| "shape offset overflow".to_string())?;
                if end > buf.len() {
                    return Err("unexpected end of file reading shape".to_string());
                }
                let raw = u64::from_le_bytes(
                    buf[pos..end]
                        .try_into()
                        .map_err(|_| "invalid shape encoding".to_string())?,
                );
                let d = usize::try_from(raw)
                    .map_err(|_| format!("tensor dimension {} is too large", raw))?;
                shape.push(d);
                pos = end;
            }
            let len_end = pos
                .checked_add(8)
                .ok_or_else(|| "data-length offset overflow".to_string())?;
            if len_end > buf.len() {
                return Err("unexpected end of file reading data length".to_string());
            }
            let raw_len = u64::from_le_bytes(
                buf[pos..len_end]
                    .try_into()
                    .map_err(|_| "invalid data-length encoding".to_string())?,
            );
            let data_len = usize::try_from(raw_len)
                .map_err(|_| format!("tensor length {} is too large", raw_len))?;
            pos = len_end;
            total_values = total_values
                .checked_add(data_len)
                .ok_or_else(|| "total tensor length overflow".to_string())?;
            if total_values > MAX_TOTAL_VALUES {
                return Err(format!(
                    "total tensor values {} exceeds limit {}",
            ...
            if expected_len != Some(data_len) {
                return Err(format!(
                    "tensor shape {:?} does not match data length {}",
                    shape, data_len
                ));
            }
            let data_bytes = data_len
                .checked_mul(8)
                .ok_or_else(|| "tensor byte length overflow".to_string())?;
            let data_end = pos
                .checked_add(data_bytes)
                .ok_or_else(|| "tensor data offset overflow".to_string())?;
            if data_end > buf.len() {
                return Err("unexpected end of file reading tensor data".to_string());
            }
            let mut data = Vec::with_capacity(data_len);
            for chunk in buf[pos..data_end].chunks_exact(8) {
                let v = f64::from_le_bytes(
                    chunk
                        .try_into()
                        .map_err(|_| "invalid tensor value encoding".to_string())?,
                );
                data.push(v);
            }
            pos = data_end;
