...
| `cargo test --all-targets` | PASS, 242 tests (148 library, 33 LSP binary, 9 frontend robustness, 52 runtime outcome) |
...
3. **Partly implemented:** complete-program evaluation now returns a structured
   ...
   `try/catch`. The payload still needs to move into `EvalResult`: 210 textual
   producer/propagation occurrences across 203 source lines remain, so migration
