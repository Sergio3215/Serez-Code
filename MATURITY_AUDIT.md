...
Current verified baseline on Windows (re-measured 2026-09-01 as the M0 milestone
baseline, at `v10.0.0` / HEAD `d8662c2`; the reproducible record with commands and
artifacts is `docs/maturity/ROADMAP_STATE.md`):

| Gate | Result |
|---|---|
| `cargo fmt --check` | PASS |
| `cargo check` | PASS, no warnings |
| `cargo clippy --all-targets` | PASS, 186 historical library warnings across 39 distinct lints, no errors (59 in `evaluator/ops.rs`, 26 in `namespaces_gui.rs`, 3 in `parser.rs`); CI does not pass `-D warnings`, so they are not a gate |
| `cargo test --all-targets` | PASS, 318 tests (180 library, 36 LSP binary, 16 frontend robustness, 79 runtime outcome, 3 diagnostic codes, 4 filesystem reach) |
| Serez test runner | PASS on **both** runners, 490 files/groups each; 0 failed; 0 skipped. Parity is now checked per category rather than only on the total: `ai 5, check 3, cli 14, e2e 91, error 63, eval 13, import 4, package-manager 15, repl 11, runner-integrity 1, rust 2, security 104, unit 164` are identical in both `serez-conformance/1` reports. `run_tests.sh` had been executing 306 of them — see the parity row below. |
...
The Rust codebase contains 54,555 lines across 63 files. The largest
files are `namespaces_gui.rs` (6,264), `parser.rs` (3,936),
`methods_tensor.rs` (3,672), `namespaces_autodiff.rs` (2,935),
`evaluator/mod.rs` (2,653), `package_manager.rs` (2,649) and
`namespaces_gui/render.rs` (2,530). One of the 63 is not in the module tree at
all: `src/test_run.rs` is a UTF-16 two-line `main` from 2026-05-07 that reads
`test.serez`, an extension the language no longer has. Nothing declares it, so it
is neither compiled nor formatted nor linted — it is tracked dead weight.
...
| Parser | medium | architectural debt (largely resolved), diagnostics | **The monolith is gone.** M1 took one 3,936-line `impl Parser` apart into 14 modules behind a 422-line façade, each extraction verified against a per-file hash of the parse tree and the diagnostic list across all 490 in-repo `.sz` files (`tests/parser_snapshot.rs`). No inverse dependencies; the horizontal edges form a DAG. Coded positional errors reach CLI and LSP through one shape and recovery is still bounded, but **recovery rules remain ad hoc**. The defect M1 surfaced — nine parser errors that never reached `take_errors()`, so the LSP showed nothing for them — was **fixed in M3.7**: all nine now carry `SZ2000`, a file, a line, a column and a caret, and nine conformance fixtures pin them, because the measurement that closed it showed those paths had **no coverage at all** in 490 files and 149 fixtures. | Done: a common frontend `Diagnostic` (M3) and uniform AST spans (M2) both landed without changing text or codes. Recovery rules are the remaining parser debt. |
...
| AST | low | architectural debt (spans resolved) | **Positions are now uniform.** M2 gave every node a `Span` — 39 of 40 structs and all 28 `Expression` variants; only `Program` is excluded, deliberately, since its extent is the file. `src/span.rs` is a leaf module carrying both the rendered `line`/`column` (normative per `lexical-grammar.md`) and byte offsets, and `Token` now has one representation of its position rather than two. Verified by `tests/ast_spans.rs`, `tests/lexer_spans.rs` and a per-file hash of the parse tree across the corpus. One broad AST is still shared by interpreter, tooling and the incomplete compiler. Spans now have a consumer: M3's `Diagnostic` carries one, and `src/render.rs` is the single place it becomes text. A runtime error still takes its position from the call stack rather than from the node, and the LSP still discards the parse tree. | Add spans uniformly before changing syntax; keep AST source-oriented and lower to separate semantic IR. |
...
| Versioning | high | bug (fixed), compatibility, documentation mismatch | Runtime is 10.0.0: `Cargo.toml` says so, `v10.0.0` is tagged on `origin` and points at HEAD. `CHANGELOG.md` does not — every one of the ~1,800 lines of work between 9.17.0 and now still sits under `## [Unreleased] — maturity hardening`, so the released version has no changelog heading. **Resolved:** the documented core-floor declaration was not merely unenforced, it was actively broken — `sz install` in a `serez-ui` project failed because `"serez-code": ">= 9.17.0"` went through the package-version identifier rules. The key is now reserved, checked against the running runtime and never fetched, with Rust and CLI regressions on both platforms. `spec/compatibility.md` states the policy that `limits.md` and `random.md` already referenced. **Remaining:** the other seven official packages declare no minimum, and there is still no separate language-specification version. | Adopt the floor across the official manifests, then decide whether a separate spec version earns its keep. |
