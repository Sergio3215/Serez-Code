...
5. Audit the remaining runtime panic/unwrap reachable from user input; add
   regression tests before replacing each one. 286 `unwrap`/`expect`/`panic!`
...
3. **Partly implemented:** complete-program evaluation now returns a structured
   ...
   `try/catch`. The payload still needs to move into `EvalResult`: 242 textual
   producer/propagation occurrences across 235 source lines remain, so migration
